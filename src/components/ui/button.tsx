...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, gaTrack, onClick, ...props }, ref) => {
    const Comp = asChild ? Slot : "button";
    
    const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
      if (gaTrack) {
        ReactGA.event(gaTrack.event_name, {
          event_category: gaTrack.event_category || "button_click",
          event_label: gaTrack.event_label,
          value: gaTrack.value,
          ...gaTrack,
        });
      }
